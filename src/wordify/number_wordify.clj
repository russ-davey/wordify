...
(def ^:private large-numbers-extended
  (reduce (fn [new-map [mag-int mag]]
            (if (> mag-int 2)
              ...
              new-map))
          (merge mappings/large-numbers {1 "ten"})
          mappings/large-numbers))
...
(defn- get-order-of-magnitude
  ...
  ([i]
   (get-order-of-magnitude i mappings/large-numbers))
  ([i number-map]
   (when-not (zero? i)
...
(defn- within-acceptable-params?
  ...
  (let [int-or-big-int (or (int? number)
        ...
        within-range (<= (get-magnitude-number number) (+ (apply max (keys mappings/large-numbers)) 2))]
